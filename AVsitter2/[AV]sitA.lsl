...
integer NOTECARD_PROGRESS_STEP = 32;
integer notecard_progress_next_update;
key reused_key;
...
integer is_notecard_loader;
integer notecard_finalized;
...
store_linkset_global_value(string field, string value)
{
    llMessageLinked(LINK_THIS, MSG_LINKSET_STORE_GLOBAL, field + LINKSET_MESSAGE_SEPARATOR + value, "");
}

store_linkset_sitter_info(integer channel, string value)
{
    if (channel >= 0)
    {
        llMessageLinked(LINK_THIS, MSG_LINKSET_STORE_SITTER, (string)channel + LINKSET_MESSAGE_SEPARATOR + value, "");
    }
}

store_linkset_button_entry(integer channel, string command, string label, string data)
{
    if (channel >= 0)
    {
        llMessageLinked(LINK_THIS, MSG_LINKSET_STORE_BUTTON, (string)channel + LINKSET_MESSAGE_SEPARATOR + command + LINKSET_MESSAGE_SEPARATOR + label + LINKSET_MESSAGE_SEPARATOR + data, "");
    }
}

store_linkset_position_entry(integer channel, string pose_name, string pos, string rot)
{
    if (channel >= 0)
    {
        llMessageLinked(LINK_THIS, MSG_LINKSET_STORE_POSITION, (string)channel + LINKSET_MESSAGE_SEPARATOR + pose_name + LINKSET_MESSAGE_SEPARATOR + pos + LINKSET_MESSAGE_SEPARATOR + rot, "");
    }
}
...
integer process_notecard_line(string raw_data)
{
    ...
    store_linkset_global_value("NOTECARD_FINALIZED", (string)notecard_finalized);
    integer marker = llSubStringIndex(raw_data, "◆");
    string data = raw_data;
    if (marker != -1)
    {
        data = llGetSubString(raw_data, marker + 1, -1);
    }
    ...
    else
    {
        ...
        string argument_data = llGetSubString(data, space_index + 1, -1);
        list raw_parts = llParseStringKeepNulls(argument_data, ["|"], []);
        integer raw_length = llGetListLength(raw_parts);
        integer idx;
        while (idx < raw_length)
        {
            parts += llStringTrim(llList2String(raw_parts, idx), STRING_TRIM);
            idx++;
        }
        if (raw_length)
        {
            part0 = llList2String(parts, 0);
            if (raw_length > 1)
            {
                part1 = llDumpList2String(llList2List(parts, 1, -1), SEP);
            }
        }
    }
    string first_char = llGetSubString(data, 0, 0);
    if (command == "SITTER")
    {
    ...
    if (command == "SITTER")
    {
        ...
        store_linkset_sitter_info((integer)part0, llDumpList2String(parts, SEP));
    ...
    if (command == "MTYPE")
    {
        ...
        store_linkset_global_value("MTYPE", (string)MTYPE);
        return forward_target;
    }
    if (command == "ROLES")
    {
        RLVDesignations = (string)parts;
        store_linkset_global_value("ROLES", RLVDesignations);
        return forward_target;
    }
    if (command == "ETYPE")
    {
        ETYPE = (integer)part0;
        store_linkset_global_value("ETYPE", (string)ETYPE);
        return forward_target;
    }
    if (command == "SELECT")
    {
        SELECT = (integer)part0;
        store_linkset_global_value("SELECT", (string)SELECT);
        return forward_target;
    }
    if (command == "WARN")
    {
        WARN = (integer)part0;
        store_linkset_global_value("WARN", (string)WARN);
        return forward_target;
    }
    if (command == "TEXT")
    {
        CUSTOM_TEXT = llDumpList2String(llParseStringKeepNulls(part0, ["\\n"], []), "\n");
        store_linkset_global_value("TEXT", CUSTOM_TEXT);
        return forward_target;
    }
    if (command == "SWAP")
    {
        SWAP = (integer)part0;
        store_linkset_global_value("SWAP", (string)SWAP);
        return forward_target;
    }
    if (command == "AMENU")
    {
        AMENU = (integer)part0;
        store_linkset_global_value("AMENU", (string)AMENU);
        return forward_target;
    }
    if (command == "HELPER")
    {
        OLD_HELPER_METHOD = (integer)part0;
        store_linkset_global_value("HELPER", (string)OLD_HELPER_METHOD);
        return forward_target;
    }
    if (command == "SET")
    {
        SET = (integer)part0;
        store_linkset_global_value("SET", (string)SET);
        return forward_target;
    }
    if (command == "KFM")
    {
        HASKEYFRAME = (integer)part0;
        store_linkset_global_value("KFM", (string)HASKEYFRAME);
        return forward_target;
    }
    if (command == "LROT")
    {
        REFERENCE = (integer)part0;
        store_linkset_global_value("LROT", (string)REFERENCE);
        return forward_target;
    }
    if (command == "BRAND")
    {
        BRAND = part0;
        store_linkset_global_value("BRAND", BRAND);
        return forward_target;
    }
    if (command == "DFLT")
    {
        DFLT = (integer)part0;
        store_linkset_global_value("DFLT", (string)DFLT);
        return forward_target;
    }
    if (command == "ONSIT")
    {
        onSit = part0;
        store_linkset_global_value("ONSIT", onSit);
        return forward_target;
    }
    if (command == "ADJUST")
    {
        ADJUST_MENU = parts;
        store_linkset_global_value("ADJUST", llDumpList2String(ADJUST_MENU, SEP));
        return forward_target;
    }
    if (reading_notecard_section)
    {
        if (first_char == "{")
        {
            integer close_brace = llSubStringIndex(data, "}");
            if (close_brace != -1)
            {
                command = llStringTrim(llGetSubString(data, 1, close_brace - 1), STRING_TRIM);
                string remainder = llStringTrim(llGetSubString(data, close_brace + 1, -1), STRING_TRIM);
                integer pos_start = llSubStringIndex(remainder, "<");
                if (pos_start != -1)
                {
                    string pos_block = llGetSubString(remainder, pos_start, -1);
                    integer pos_end = llSubStringIndex(pos_block, ">");
                    if (pos_end != -1)
                    {
                        string pos = llStringTrim(llGetSubString(pos_block, 0, pos_end), STRING_TRIM);
                        string rot_block = llStringTrim(llGetSubString(pos_block, pos_end + 1, -1), STRING_TRIM);
                        integer rot_start = llSubStringIndex(rot_block, "<");
                        if (rot_start != -1)
                        {
                            rot_block = llGetSubString(rot_block, rot_start, -1);
                            integer rot_end = llSubStringIndex(rot_block, ">");
                            if (rot_end != -1)
                            {
                                string rot = llStringTrim(llGetSubString(rot_block, 0, rot_end), STRING_TRIM);
                                ...
                                store_linkset_position_entry(forward_target, command, pos, rot);
                            }
                        }
                    }
                }
        ...
        else
        {
            ...
            if (command == "POSE" || command == "SYNC" || command == "MENU" || command == "TOMENU" || command == "BUTTON")
            {
                ...
                store_linkset_button_entry(forward_target, command, part0, part1);
            }
        }
    }
    return forward_target;
...
{
    if (notecard_finalized)
    {
        return;
    }
    notecard_finalized = TRUE;
    store_linkset_global_value("NOTECARD_FINALIZED", (string)notecard_finalized);
    store_linkset_global_value("FIRST_POSENAME", FIRST_POSENAME);
    store_linkset_global_value("FIRST_ANIMATION_SEQUENCE", FIRST_ANIMATION_SEQUENCE);
    store_linkset_global_value("FIRST_POSITION", (string)FIRST_POSITION);
    store_linkset_global_value("FIRST_ROTATION", (string)FIRST_ROTATION);
    store_linkset_global_value("DEFAULT_POSITION", (string)DEFAULT_POSITION);
    store_linkset_global_value("DEFAULT_ROTATION", (string)DEFAULT_ROTATION);
    store_linkset_global_value("CURRENT_POSE_NAME", CURRENT_POSE_NAME);
    store_linkset_global_value("CURRENT_ANIMATION_SEQUENCE", CURRENT_ANIMATION_SEQUENCE);
    store_linkset_global_value("MALE_POSENAME", MALE_POSENAME);
    store_linkset_global_value("FEMALE_POSENAME", FEMALE_POSENAME);
    store_linkset_global_value("FIRST_MALE_ANIMATION_SEQUENCE", FIRST_MALE_ANIMATION_SEQUENCE);
    store_linkset_global_value("FIRST_FEMALE_ANIMATION_SEQUENCE", FIRST_FEMALE_ANIMATION_SEQUENCE);
    store_linkset_global_value("GENDERS", llDumpList2String(GENDERS, SEP));
    store_linkset_global_value("SITTERS_COUNT", (string)llGetListLength(SITTERS));
    if (SCRIPT_CHANNEL)
    {
        sittargets();
    }
...
default
{
    state_entry()
    {
        ...
        notecard_progress_next_update = 1;
        if (SCRIPT_CHANNEL)
        {
        ...
        llMessageLinked(LINK_THIS, MSG_LINKSET_CLEAR, (string)SCRIPT_CHANNEL, "");
        store_linkset_global_value("script_channel", (string)SCRIPT_CHANNEL);
        store_linkset_global_value("script_name", llGetScriptName());
        store_linkset_global_value("version", version);
        store_linkset_global_value("notecard_name", notecard_name);
        store_linkset_global_value("memory_script", memoryscript);
    ...
    dataserver(key query_id, string data)
    {
        if (query_id == notecard_query)
        {
            if (data == EOF)
            {
                ...
                if (is_notecard_loader)
                {
                    llMessageLinked(LINK_SET, MSG_NOTECARD_DONE, "", (string)SCRIPT_CHANNEL);
                }
                return;
            }
            if (is_notecard_loader)
            {
                notecard_lines_read++;
                if (notecard_lines_read >= notecard_progress_next_update)
                {
                    llSetText("Loading (" + (string)notecard_lines_read + " lines)", <1,1,0>, 1);
                    notecard_progress_next_update = notecard_lines_read + NOTECARD_PROGRESS_STEP;
                }
